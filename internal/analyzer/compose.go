...
func (a *ComposeAnalyzer) checkVolumes(
	...
	for _, volNode := range volumesNode.Content {
		...
		if rules.IsDockerSocket(hostPath) {
			...
			f := finding.New(
				"CIS-5.31",
				"Service '"+serviceName+"' mounts Docker socket: "+hostPath,
				finding.SeverityCritical,
				target,
			).
				WithDescription(
					"Mounting " + hostPath +
						" gives the container full control over the Docker daemon.",
				).
				WithCategory(string(CategoryCompose)).
				WithLocation(loc).
				WithRemediation(
					"Do not mount /var/run/docker.sock inside containers.",
				)
